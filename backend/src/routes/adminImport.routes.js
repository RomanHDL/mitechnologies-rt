    ...
    try {
        ...
        await sequelize.transaction(async(t) => {
            ...
            if (pallets.length) {
                const days = [
                    ...new Set(
                        pallets.map((r) => toISODate(r.day || r.Day || r.Fecha || r.FECHA)).filter(Boolean)
                    ),
                ];
                ...
                if (safe.length) {
                    ...
                    // ✅ realtime (si tienes middleware res.locals.emit en server.js)
                    res.locals.emit = res.locals.emit || [];
                    res.locals.emit.push({ event: 'palletDashboard:update', data: { at: new Date().toISOString(), source: 'import-excel' } });
                    res.locals.emit.push({ event: 'dashboard:update', data: { at: new Date().toISOString(), source: 'import-excel' } });
                }
            }
            ...
            if (details.length) {
                const days = [
                    ...new Set(
                        details.map((r) => toISODate(r.day || r.Day || r.Fecha || r.FECHA)).filter(Boolean)
                    ),
                ];
            ...
            if (prodReqs.length) {
                for (const r of prodReqs) {
                    ...
                    await ProductionRequest.create({ area, subarea, items: [{ sku, qty }], note, status: statusOk, requestedByUserId }, { transaction: t });

                    result.ProductionRequests.inserted++;
                }

                res.locals.emit = res.locals.emit || [];
                res.locals.emit.push({ event: 'production:update', data: { at: new Date().toISOString(), source: 'import-excel' } });
            }
