...
router.post('/login', validate(loginSchema), async (req, res, next) => {
  try {
    ...
    if (!user || !user.isActive) {
      await AuthLog.create({ userId: user?.id || null, email, event: 'LOGIN_FAIL', ...meta });
      return res.status(401).json({ message: 'Credenciales inválidas' });
    }

    if ((user.employeeNumber || '').trim() !== String(employeeNumber).trim()) {
      await AuthLog.create({ userId: user.id, email, event: 'LOGIN_FAIL', ...meta });
    ...
    await AuthLog.create({ userId: user.id, email, event: 'LOGIN_SUCCESS', ...meta });
